                ...
                if (idToken != tokens.end())
                {
                    ...
                    if (type->second == "EDGE")
                    {
                        ...
                        if (parentToken != tokens.end() && childToken != tokens.end())
                        {
                            ...
                            (*parentNode)->AddEdgeToChildNode(std::move(edge)); // QUESTION: Instead of passing a std::unique_ptr<GraphEdge> edge, if pass a
                                                                                // std::unique_ptr<GraphEdge> &edge reference, what would be the difference?
                                                                                // I know that would be an Lvalue reference instead of an Rvalue, but the compiler 
                                                                                // would re-allocate memory for that reference?
                        }
